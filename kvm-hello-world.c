...
# define EFER_SCE 1
# define EFFR_LME (1U << 8)
# define EFFR_LMA (1U << 10)
# define EFFR_NXE (1U << 11)
...
int run_vm(struct vm *vm, struct vcpu *vcpu, size_t sz)
{
    ...
    check:
        ...
        if (regs.rax != 42) {
            printd("Wrong result:{E,R,}AX is %lld\n", regs.rax);
            return 0;
        }
...
static void setup_paged_32bit_mode(struct vm *vm, struct kvm_sregs *sregs)
{
    ...
    /* a single 4mb page to cover the memeory region */
    pd[0] = PDE32_PRESENT | PDE32_RW | PDE32_USER | PDE32_PS;
    /* others PDES are left zeros, meaning not present. */
...
int run_paged_32bit_mode(struct vm *vm, struct vcpu *vcpu)
{
    ...
    setup_protected_mode(&sregs);
    setup_paged_32bit_mode(vm_&sregs);
    if (ioctl(vcpu->fd, KVM_SET_SREGS, &sregs) < 0) {
        perror("KVM_SET_SREGS");
        exit(1);
    }
...
# 从下往上看，下面一层可以支撑上面一层，或者说通过下面的一层的功能可以实现上一层的功能，可以说，
# 可以划分层次，两个层次之间可以形成一个功能结构
static void setup_long_mode(struct vm *vm, struct kvm_sregs *sregs)
{
    unit64_t pml4_addr = 0x2000;
...
# 代码与现实是同构的，例如我要去学校，学校这个词包括了学校里的许多事情
...
int main(int argc, char **argv)
{
    ...
    while((opt = getopt(argc, argv, "rsp1")) != -1){
       switch (opt) {
       case 'r':
           mode = REAL_MODE;
           break;
